...
function PostList() {
  ...
  if (loading) return <p>로딩 중...</p>;
    ...
    for(let i = 0; i < data.listPost.length; i++) {
      elements.push(
        <li>
          <PostElement onClick={onClickPost} id={data.listPost[i]._id} title={data.listPost[i].Title} contents={data.listPost[i].Contents} />
    ...
    return (
      <div className="body">
        <Link className="listbutton" to='/Editor'>Post작성</Link>
